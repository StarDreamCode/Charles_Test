...
export default class BackgroundAdapter extends Component {
    onLoad() {
        ...
        // this.node.scale = Math.max(view.getCanvasSize().width / this.node.width, view.getCanvasSize().height / this.node.height);
        ...
        // 2. 基于第一步的数据，再做缩放适配
        this.node.scale = Math.max(view.getCanvasSize().width / realWidth, view.getCanvasSize().height / realHeight);
